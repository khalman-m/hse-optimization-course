...
def test_log_reg_oracle_calls():
    ...
    oracle.grad(x)
    oracle.hess(x)
    eq_(counters, {'Ax': 3, 'ATx': 1, 'ATsA': 1})
    ...
    oracle.func(x)
    oracle.grad(x)
    eq_(counters, {'Ax': 2, 'ATx': 1, 'ATsA': 0})
    ...
    oracle.grad(x)
    oracle.hess(x)
    eq_(counters, {'Ax': 2, 'ATx': 1, 'ATsA': 1})
    ...
    eq_(counters, {'Ax': 6, 'ATx': 2, 'ATsA': 0})
    ...
    oracle.func(x + 3 * d)
    oracle.grad(x + 3 * d)
    eq_(counters, {'Ax': 8, 'ATx': 3, 'ATsA': 0})

    ###########################
    ## Test optimized oracle ##
    ###########################
    # Single func
    (matvec_Ax, matvec_ATx, matmat_ATsA, counters) = get_counters(A)
    ...
    eq_(counters, {'Ax': 2, 'ATx': 0, 'ATsA': 0})
    ...
    oracle.grad(x)
    oracle.hess(x)
    eq_(counters, {'Ax': 1, 'ATx': 1, 'ATsA': 1})
    ...
    oracle.func(x + 3 * d)
    oracle.grad(x + 3 * d)
    eq_(counters, {'Ax': 2, 'ATx': 2, 'ATsA': 0})
